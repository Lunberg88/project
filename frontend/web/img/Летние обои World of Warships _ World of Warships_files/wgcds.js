(function (window) {
    ...
    WGCDS.sendRequest = function (url, data, callbacks) {
        ...
        xhr.setRequestHeader('Accept', 'application/vnd.wg.cds+json;version=2');
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    
        xhr.onload = function () {
            typeof callbacks.onSuccess == 'function' && callbacks.onSuccess(xhr);
        };
        xhr.onerror = function () {
            typeof callbacks.onError == 'function' && callbacks.onError(xhr);
        };
        xhr.onloadend = function () {
            typeof callbacks.onComplete == 'function' && callbacks.onComplete(xhr);
        };
    ...
    function Loader(selector, config) {
        this.selector = selector || '.js-wgcds-content-item';
        this.elements = document.querySelectorAll(this.selector);
        this.data = [];
        this.uaData = this._parseUA();
        this._extendConfig(config || {});
    
        var parameters = Loader.defaults.parameters;
        for (var i = 0, l = this.elements.length; i < l; i++) {
            var element = this.elements[i];
            var elData = {};
            for (var parameter in parameters) {
                var param = element.getAttribute('data-' + this.config.pluginPrefix + '-' + parameter);
                elData[parameter] = param != null && param !== '' ? param : this.config.parameters[parameter];
            }
    ...
    Loader.defaults = {
        ...
        callbacks: {
            onResponse: null,
            onSuccess: null,
            onError: function (element, status, message) {
                console.error('Error on requesting content: [' + status + '] "' + message + '"');
            },
            onEmptyContent: null
        }
    };
    ...
    Loader.prototype._mergeFlat = function (config, name, type) {
        var defaults = Loader.defaults[name];
        for (var option in defaults) {
            ...
            this.config[name][option] = configOpt || defaults[option];
        }
    };
    
    Loader.prototype._extendConfig = function (config) {
        this.config = {
            pluginPrefix: config.pluginPrefix || Loader.defaults.pluginPrefix,
            requestOnInit: config.requestOnInit !== void(0) ? config.requestOnInit : Loader.defaults.requestOnInit,
            parameters: {},
            callbacks: {}
        };
    ...
    Loader.prototype._requestContent = function (i) {
        var self = this;
        var data = this.data[i];
        var element = this.elements[i];
    
        var url = data.protocol + '//' + data.host + data.path;
                ...
                if (xhr.responseText) {
                    try {
                        ...
                        if (response.status === 'ok' && response.content) {
                            ...
                            self.config.callbacks.onSuccess && self.config.callbacks.onSuccess.call(self, element, response);
                        } else if (response.status === 'ok' && !response.content) {
                            self.config.callbacks.onEmptyContent && self.config.callbacks.onEmptyContent.call(self, element);
                        } else if (response.status === 'error' && response.error) {
                            self.config.callbacks.onError && self.config.callbacks.onError.call(self, element, xhr.status, response.error);
                        } else {
                            self.config.callbacks.onError && self.config.callbacks.onError.call(self, element, xhr.status, 'unexpected response');
                        }
                    } catch (e) {
                        console.error(e);
                        self.config.callbacks.onError && self.config.callbacks.onError.call(self, element, xhr.status, 'malformed response:' + e);
                    }
                } else {
                    self.config.callbacks.onError && self.config.callbacks.onError.call(self, element, xhr.status, 'empty response');
                }
            },
            onError: function (xhr) {
                self.config.callbacks.onError && self.config.callbacks.onError.call(self, element, xhr.status, xhr.statusText);
            },
            onComplete: function (xhr) {
                self.config.callbacks.onResponse && self.config.callbacks.onResponse.call(self, element, xhr);
            }
        };
    ...
    Loader.prototype.update = function (config, noRequest) {
        ...
        for (var i = 0, l = this.data.length; i < l; i++) {
            ...
            for (var parameter in elData) {
                var param = parameters[parameter];
                elData[parameter] = param !== void(0) ? param : elData[parameter];
            }
        }
    
        for (var cb in callbacks) {
            this.config.callbacks[cb] = callbacks[cb];
        }
